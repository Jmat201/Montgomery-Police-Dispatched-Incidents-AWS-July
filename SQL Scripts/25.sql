...
SELECT 
    ...
    CASE 
        WHEN city = 'SILVER SPRING' THEN ROUND(count_incidents / 33.0 * 100, 1)
        WHEN city = 'ROCKVILLE' THEN ROUND(count_incidents / 15.0 * 100, 1)
        WHEN city = 'GAITHERSBURG' THEN ROUND(count_incidents / 12.0 * 100, 1)
        WHEN city = 'GERMANTOWN' THEN ROUND(count_incidents / 6.0 * 100, 1)
...
ORDER BY 
    count_incidents_percentage DESC limit 10 ;
